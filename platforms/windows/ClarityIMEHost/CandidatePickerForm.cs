...
/// <summary>IME candidate picker: top recommendation = one-click send; alts below.</summary>
sealed class CandidatePickerForm : Form
{
        ...
        string mode = "default")
    {
        ...
        if (_options.Count > 0)
        {
            ...
            {
                Text = $"发送推荐 · [{LabelDisplay(top.Label)}]\r\n{top.Text}",
                Height = 76,
                Dock = DockStyle.Top,
                TextAlign = ContentAlignment.MiddleLeft,
                Padding = new Padding(12, 8, 12, 8),
                BackColor = UiTheme.SuccessBg,
                FlatStyle = FlatStyle.Flat,
            };
            sendTop.FlatAppearance.BorderColor = UiTheme.SuccessBorder;
            ...
            if (_options.Count > 1)
            {
                ...
                for (var i = 1; i < _options.Count; i++)
                {
                    ...
                    var btn = new Button
                    {
                        Text = $"{i + 1}. [{LabelDisplay(o.Label)}] {o.Text}",
                        Dock = DockStyle.Top,
                        Height = 48,
                        TextAlign = ContentAlignment.MiddleLeft,
        ...
        bad.Click += (_, _) =>
        {
            using var fb = new FeedbackForm(raw, _nbest, _options, _mode);
            fb.ShowDialog(this);
        };
        ...
        string mode = "default")
    {
        ...
        using var f = new CandidatePickerForm(raw, options, nbest, mode);
        return f.ShowDialog() == DialogResult.OK ? f.SelectedText : null;
    }

    static string LabelDisplay(string label) => label switch
    {
        "standard" => "标准",
        "structured" => "结构化",
        "inline" => "单行",
...
/// <summary>Quick feedback when no candidate fits ("骂它").</summary>
sealed class FeedbackForm : Form
{
    public FeedbackForm(
        string raw,
        string[]? nbest = null,
        ...
        string mode = "default")
    {
        ...
        ok.Click += (_, _) =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(note))
            {
                try
                {
                    var url = new CoreClient().Feedback(
                        raw,
                        $"[user_feedback] {note}",
                        nbest,
                        candidates,
