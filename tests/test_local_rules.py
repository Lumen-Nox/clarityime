...
from clarityime.clarify.details import details_preserved
from clarityime.clarify.listener_adapt import (
    _words_preserved,
    adapt_for_listener,
    adapt_with_report,
)
...
class LocalRulesTests(unittest.TestCase):
    ...
    def test_dual_output_words_preserved(self) -> None:
        ...
        self.assertTrue(details_preserved(original, for_listener))
    ...
    def test_d_type_vs_s_type_layout_differs(self) -> None:
        ...
        d_res = adapt_with_report(original, d)
        s_res = adapt_with_report(original, s)
        d_out, s_out = d_res.text, s_res.text
        # Baseline = after oral-noise removal; that is what layout must preserve.
        self.assertTrue(_words_preserved(d_res.baseline, d_out))
        self.assertTrue(details_preserved(original, s_out))
    ...
    def test_candidates_contact_dual_labels(self) -> None:
        ...
        self.assertTrue(details_preserved(orig, hear))
